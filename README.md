...
* Scatterplot: Case Incidence Counts vs. Population 2020

![](https://i.imgur.com/9XAtIxU.png)
These graphs compare the age adujusted incident rates compared to the population and the poverty rates of each county. Although the visualizations for incidence rates versus population and poverty had similar shapes, we did not observe much correlation between the features.
...
Ultimately, we were not able to fully complete our goal of creating a machine learning model to predict incidence of Breast Cancer diagnoses in US counties. Despite our extensive data cleaning work, our correlation coefficients were ultimately an indicatorthat our models may not be as useful as we hoped they would be. This came to fruititon when we created both, the Linear Regression model and the Bernouli Model. In the end, though, we were able to see that the results in the literature that applied to early stage Breast Cancer may not apply to late stage as well, or data that includes counts for late stage incidences. 
If we had more time, it would be useful to attempt to further debug our code to test and train the Bernouli Model. Moreover, it could be helpful to include additional indicators of affluence, like race, in further research, just as the literature we examined did.
